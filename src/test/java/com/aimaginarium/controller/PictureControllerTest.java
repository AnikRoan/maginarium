...
class PictureControllerTest {
    ...
    @Test
    void savePictureAndDetails() throws Exception {
        DetailsAndPictureDto detailsAndPictureDto = new DetailsAndPictureDto();

        detailsAndPictureDto.setPictureDto(pictureDto);
        detailsAndPictureDto.setPictureDetailsDto(pictureDetailsDto);


        mockMvc.perform(post(PATH+"/save_picture_and_details")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(detailsAndPictureDto)))
                .andExpect(status().isOk());
    }
    ...
    void updatePicture() throws Exception {
        ...
        verify(mockPictureService, times(1)).updatePicture(pictureDto, id);
    }

    @Test
    void updatePictureDetails() throws Exception {
        mockMvc.perform(put(PATH+"/update_details/{id}", id)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(pictureDetailsDto)))
                .andExpect(status().isOk());

        verify(mockPictureDetailsService, times(1)).updateDetails(pictureDetailsDto, id);
    }
}